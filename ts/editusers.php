...
?>
                            Records to View:
                            <INPUT type=text name='limit' size="3" value="<?PHP echo $_SESSION['limit']; ?>" /> Table Search:
                            <INPUT type=text name='search' size="30" value="<?PHP echo $_POST['search']; ?>" />
    ...
    if($_GET['action']=='savechanges') {
        //Announce Saved (although if there is an error later, this will not be true, we want to give an indication to the user that their prior button press had an effect ...) ?>
                    <h3 style=" color: green">Record Changed</h3>
                    <p style=" color: green">If you have changed your mind, you may press REVERT below to revert to the prior version of this record.
                        <br/>The PRIOR data is displayed <i>above</i> the Revert button.
                        <br/>The NEW data is displayed <i>below</i> the Revert button (with the rest of the records).</p>
...
// Get result from query
    ...
    ?>
        ...
        <head>
            <title>Bouncing for Login ...</title>
